...
The architecture is desgined to make the process of large-scale log data more efficiently while enabling to
...
\begin{enumerate}[label=\arabic*.]
    ...
    \item Modular Service Design
          ...
          Each component in the system is desgined as an independent module with clear responsibilities to maintain or test
          more easily, and also increase the ability of horizontally scaling of individual components.

    \item Data Layer Separation

          The system create a clear seperation between metadata management and object storage that using
          MinIO for large-scale objects and PostgreSQL for logs structured metadata. This design optimizes
          the performace by addressing the data access pattern problem where PostgreSQL is suitable for
          structured queries and indexing whereas MinIO offer hight throughput and cost efficiency for
          actual log storage.
...
The system uses a layered, distributed architecture which is desgined to support efficient log ingestion and
...
\begin{enumerate}[label=\arabic*.]
    ...
    \item Service Layer
          ...
          and LLMs analysis.
          By seperating functions into discreate services, the architecture allows each service to
    ...
    \item Task Management Layer
          ...
          To support asynchronous processing as I mentionen in section 3.2.1, the system also create a
