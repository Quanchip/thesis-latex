...
\begin{enumerate}[label=\alph*.]
    ...
    \item Introduction to distributed task processing
          ...
          Distributed task processing has become increasingly imprtant in modern system because
          this technology allows all tasks are distributed efficiently accross multiple nodes.
          This approach enhances system scalability, performance, and enable entensive computation
          ...
          process complete. This issue exeed typical HTTP timeou thresholds, therefor degrading
    ...
    \item Task Implementation
          ...
          \begin{enumerate}[label=\arabic*.]
              ...
              \item Log Processing Task
                    ...
                    The system implementes a two-stage processing pipeline through chained Celery task.
                    ...
                    The Celery worker first retrieve the log file from MinIO object strorage and decode
                    UTF8 centent, then invoke the Drain parser service for template extraction.
              ...
              \item Machine Learning Analysis Task
                    ...
                    task, worker retreive structured log CSV file from MinIO, the pre-trained model is
...
the implementation will provide a contructor for \texttt{DrainParserService} with several parameters
...
\begin{figure}[H]
    \centering
    \includegraphics[width=1\textwidth]{drain-imple.png}
...
extraction is applied at two distinct phases: (1) During trainning phase, where
...
\begin{enumerate}[label=\alph*.]
    ...
    \item Training Phase
          ...
          Firgure shows the configuration for feature extraction during the offline trainning
          ...
          anomalt label at sequence level not entry level. Once the
          ...
          whe count the occurence the number of single log event to form the event
          count vector. Finally every count vector are formed an event count metrix $X$,
...
by Loglizer project \cite{7774521} as mentioned erlier in this study. Each algorithm is
...
will provide human-readable explainations and recommendations for marked anomalies.
...
\begin{enumerate}[label=\arabic*.]
    ...
    \item Context Preparation and Session Retrieval
          ...
          To effectively detect anomaly we can not just examin a single log entry, it is
          ...
          involves four steps which is shown in Firgure \ref{fig:context}.
          First, the system queries the database to find the file path of
          CSV file in MinIO. Next, the CSV is dowloaded from MinIO using chunk
          approach to prevent unecessary memory usage. The parsed log entries are
...
\begin{enumerate}[label=\arabic*.]
    ...
    \item System Prompt: Role Definition
          ...
          format requirment for LLM to JSON format only. This approach is necessary because
    ...
    \item User Prompt
          ...
          expore the all the related event to give a more concise explanations and explanations
          actions.
...
The frontend of the system organizes the components by grouping them by functionality. This aproach
...
seperation between components to make it easy to develop and maintain. For instance,
authentication-related components such as Login.tsx, Register.tsx, AuthContext.tsx
are seperated from analysis components like LogUpload.tsx or Results.tsx. In addition,
...
system. The analysis focuses on the overall effectiveness of the machine learing models
for log anomaly detection, the overall performance of the system and the quality of
LLM generated explainations.

\section{Machine learing models evaluations}

To measures the performance of each model, I use  precision, recall and F-measure,
which are the most commonly usedme trics, to evaluate the
...
measures the proportion of how many anomolies are correctly identified in
those reported.
...
Based on the experiment results that capsulates in Figure , The Decision Tree
model demonstates that most superior performance among all approaches.
...
decrease its ability to dectect diverse pattern. Forest and Logistic Regression
...
contains explanations why a log sequence is lableed anomaly, root causes analysis,
severity level and recommended actions. In most cases, the recommendations are sound realiable
and detail enough.
